...
//   while ( UCSR0A & (0b10000000) != 0){ // Check to see RXC0 register is 1 or 0 //
//     buffer[counter++] = UDR0;
//   }
...
void loop(){
  writeUART( "Hello World" , 11 );
  //readUART( buffer );
  writeUART( buffer, 11 );
  // writeSerial( "Hello World", 11);
  // readSerialLib(buffer);
  // writeSerial( buffer, 11 );
}
